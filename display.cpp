...
void display_update(uint8_t *temps, uint8_t *humids, uint16_t size, uint16_t offset) {
    if (error) return;
    uint16_t stop = (size>(uint16_t)tft.width()) ? (uint16_t)tft.width() : size;
    
//     display_clear();
    for (uint16_t dspl_idx = 0; dspl_idx < stop; dspl_idx++) {
        display_update_step(temps, humids, size, offset, dspl_idx, 0);
    }
}
...
void display_update_step(uint8_t *temps, uint8_t *humids, uint16_t size, uint16_t offset, uint16_t step, int cursor_on) {
    if (step >= tft_width || error) return;

    uint8_t ht = temps[(step+offset)%size];
    uint8_t hh = humids[(step+offset)%size];
    uint16_t w = tft_width-1-step;

    tft.drawFastVLine(w, 0, tft_height, BLACK);
    if (cursor_on) {
        tft.drawFastVLine(w-1, 0, tft_height-16, MAGENTA);
    }
    tft.drawPixel(w, 150, GREEN);
    //tft.drawPixel(step, 128, YELLOW);
    tft.drawPixel(w, 200, RED);
    
    if (ht <= tft_height) {
        tft.drawPixel(w, ht-1, BLUE);
        tft.drawPixel(w, ht,   0x03FF);
        tft.drawPixel(w, ht+1, BLUE);
    }
    if (hh <= tft_height) {
        tft.drawPixel(w, hh,   YELLOW);
    }
}
